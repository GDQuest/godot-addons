...
async function main() {
  try {
    const [LICENSE_CONTENTS, ADDONS_DIRS] = await Promise.all([
      fs.readFile(LICENSE_FILE_NAME, { encoding: 'utf8' }),
      fs.readdir(ADDONS_DIR),
    ])
    let license = LICENSE_CONTENTS.split(/[\r\n]+/).slice(0, 1).join().replace(/\s+license/i, '')
    ...
    for (const DIR of ADDONS_DIRS) {
      ...
      await Promise.all([
        fs.writeFile(path.join(ADDON_PATH, ADDON_PACKAGE_JSON_FILE_NAME), JSON.stringify(ADDON_PACKAGE_JSON)),
        fs.cp(LICENSE_FILE_NAME, path.join(ADDON_PATH, LICENSE_FILE_NAME)),
     ])
    }
